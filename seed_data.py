...
def seed_database():
    ...
    class_ids = []
    for cls in classes:
        cursor.execute("SELECT class_id FROM classes WHERE class_name = ?", (cls["name"],))
        row = cursor.fetchone()
        if row:
            class_ids.append(row["class_id"])
            logger.info(f"Class '{cls['name']}' already exists.")
        else:
            cursor.execute("""
    ...
    for i, s in enumerate(students_data):
        cursor.execute("SELECT student_id FROM students WHERE first_name = ? AND last_name = ?", (s["first"], s["last"]))
        row = cursor.fetchone()
        
        if row:
            student_id = row["student_id"]
            logger.info(f"Student {s['first']} already exists.")
        else:
            cursor.execute("""
            ...
            logger.info(f"Created student: {s['first']} {s['last']}")
            
            # Add Allergies
            if i == 0: # Alice has allergies
                cursor.execute("""
        ...
        cursor.execute("INSERT OR IGNORE INTO student_parents (student_id, user_id) VALUES (?, ?)", (student_id, pid))
        logger.info(f"Linked student {student_id} to parent {pid}")
            
        # Enroll in Class
        class_idx = i % len(class_ids)
        cid = class_ids[class_idx]
        cursor.execute("INSERT OR IGNORE INTO student_classes (student_id, class_id) VALUES (?, ?)", (student_id, cid))
    ...
    meals = [
        ...
        (school_id, class_ids[0], today, "Yogurt and granola", "Chicken salad", "Grilled cheese", teacher_id, now),
